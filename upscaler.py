...
def find_images_in_directory(directory: Path, recursive: bool = False) -> List[Path]:
    ...
    # Use rglob('*') for recursive search or iterdir() for non-recursive
    # This avoids multiple directory traversals
    if recursive:
...
def upscale_single_image(
    input_path: Path, output_path: Path, scale_factor: int, gpu: bool = True
) -> bool:
    """
...
def upscale_batch(
    ...
    for i, img_path in enumerate(images, 1):
        ...
        if output_dir:
            output_path = (
                output_dir
                / f"{img_path.stem}_upscaled_x{scale_factor}{img_path.suffix}"
            )
        else:
            output_path = (
                input_dir / f"{img_path.stem}_upscaled_x{scale_factor}{img_path.suffix}"
            )
...
Examples:
  ...
  %(prog)s -d /path/to/images -o /path/to/output -s 2 --recursive
    ...
    input_group.add_argument("-i", "--input", type=Path, help="Input image file")
    input_group.add_argument(
        "-d", "--directory", type=Path, help="Directory containing images to upscale"
    )

    # Output options
    parser.add_argument(
        "-o", "--output", type=Path, help="Output file or directory for upscaled images"
    )
    ...
    parser.add_argument(
        "--cpu", action="store_true", help="Use CPU instead of GPU for processing"
    )
    ...
    try:
        subprocess.run(["upscayl-ncnn", "--help"], capture_output=True, check=True)
    ...
    if args.input:
        ...
        if args.output is None:
            ...
            output_path = (
                args.input.parent
                / f"{args.input.stem}_upscaled_x{args.scale}{args.input.suffix}"
            )
        else:
